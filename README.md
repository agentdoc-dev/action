...
| `cloud-proposal-resolver` | — | Cloud proposal resolver prefix `https://<cloud-origin>/workspaces/<id>`; non-empty marks the repository connected (commit trailer, PR link, `adoc.git_proposal_references.v0` block). Empty keeps disconnected delivery unchanged. Connected `commit` delivery first reads the classic protection of the source branch and the rulesets whose rules apply to it and refuses (`delivery_check_failed`) unless absence of protection or a bypass-free profile is proven; the `github-token` credential therefore needs repository administration read, which the workflow `GITHUB_TOKEN` cannot be granted. |
