...
request_keys='["assessment_digest","base_repository","base_revision","context_request","context_request_digest","evaluation_date","head_repository","head_revision","pull_request","request_digest","untrusted_source","version"]'
...
[ "$(jq -r .expires_at "$authorization")" ">" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" ] \
...
if ! jq -e --arg base_repo "$base_repo" --arg head_repo "$head_repo" \
  --arg base "$base" '
    ...
    and .head.repo.full_name == $head_repo and .base.sha == $base
...
if ! env -u GH_TOKEN -u GITHUB_TOKEN GIT_TERMINAL_PROMPT=0 \
  ...
  fail_trusted trusted.head_fetch_failed 'exact untrusted head fetch failed' \
    'Ensure the fork head is publicly readable and retry.'
fi
