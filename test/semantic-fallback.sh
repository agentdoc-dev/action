...
{"schema_version":"adoc.semantic_executor_request.v0","request_id":"primary","adapter":{"kind":"generic","provider":"local","model":"local-v1","endpoint_class":"local","endpoint_id":"local","executor_digest":"$D","model_digest":"$D","config_digest":"$D"},"context":{"schema_version":"adoc.semantic_context.v0","context_digest":"$D"}}
JSON
...
jq -n --slurpfile request "$request" --arg digest "$DIGEST" '{
  outcome:"completed",assessment_digest:$digest,adapter:$request[0].adapter,
...
for mode in process_fail invalid timeout; do
  run_chain "$mode"
...
! jq -e -f "$ROOT/scripts/semantic-status.jq" \
