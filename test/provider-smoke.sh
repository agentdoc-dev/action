...
export INPUT_ADOC_VERSION=v0.4.0 INPUT_WORKING_DIRECTORY=.
...
export INPUT_PROVIDER_TIMEOUT_SECONDS=600
...
jq -n --arg version "v$version" --arg sha "$binary_sha" '{
  requested_version:"v0.4.0",resolved_version:$version,binary_sha256:$sha
...
jq -e '.status != "error"' "$ADOC_RUN_DIR/semantic-status.json" >/dev/null
jq -c '{
