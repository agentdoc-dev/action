...
export INPUT_ADOC_VERSION=v0.3.1 INPUT_WORKING_DIRECTORY=.
...
jq -n --arg requested v0.3.1 --arg resolved v0.3.1 \
...
jq -e --arg base "$base" --arg head "$head" '
  ...
  and .toolchain.adoc.resolved_version == "v0.3.1"' "$receipt_path" >/dev/null

ENFORCEMENT=advisory SCOPE=full ADOC_VERSION=v0.3.1 "$ROOT/scripts/compose.sh"
