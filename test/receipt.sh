...
v9_evidence="$ROOT/.github/workflows/g1a-v7-evidence.yml"
grep -Fq 'types: [opened]' "$v9_evidence"
grep -Fq 'cancel-in-progress: false' "$v9_evidence"
grep -Fq 'needs: bind' "$v9_evidence"
grep -Fq 'g1a-binding:v9:${GITHUB_RUN_ID}:${GITHUB_RUN_ATTEMPT}' "$v9_evidence"
grep -Fq 'split("\n") | index($marker) != null' "$v9_evidence"
grep -Fq '.author_association | IN' "$v9_evidence"
grep -Fq 'uses: agentdoc-dev/action@17da62659dc164b98ccdf0f4455c7628b58cd154' "$v9_evidence"
grep -Fq 'adoc-version: v0.3.4' "$v9_evidence"
grep -Fq 'retention-days: 90' "$v9_evidence"
