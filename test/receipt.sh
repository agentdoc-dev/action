...
v8_evidence="$ROOT/.github/workflows/g1a-v7-evidence.yml"
grep -Fq 'types: [opened]' "$v8_evidence"
grep -Fq 'cancel-in-progress: false' "$v8_evidence"
grep -Fq 'needs: bind' "$v8_evidence"
grep -Fq 'g1a-binding:v8:${GITHUB_RUN_ID}:${GITHUB_RUN_ATTEMPT}' "$v8_evidence"
grep -Fq 'split("\n") | index($marker) != null' "$v8_evidence"
grep -Fq '.author_association | IN' "$v8_evidence"
grep -Fq 'uses: agentdoc-dev/action@17da62659dc164b98ccdf0f4455c7628b58cd154' "$v8_evidence"
grep -Fq 'adoc-version: v0.3.4' "$v8_evidence"
grep -Fq 'retention-days: 90' "$v8_evidence"
