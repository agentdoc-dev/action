...
emitted_codes() {
  local dir="$1"
  grep -rhoE 'https://api\.github\.com\b|\b(action|attestation|api|workspace|egress|governance|ingest|writeback|migration)\.[A-Za-z0-9_]+\b|\b(adoc|agentdoc)\.[A-Za-z0-9_.]+\.v[0-9]+\b' \
    "$dir" --exclude-dir=.git --exclude-dir=test --exclude-dir=__pycache__ 2>/dev/null |
    grep -vEx 'action\.yml|egress\.py|https://api\.github\.com' | # manifest/helper file names, not wire codes
    sort -u
}
...
scan() {
  ...
  if grep -rnE '(action|attestation|api|workspace|egress|governance|ingest|writeback|migration)\.(\$|\{)' "$dir" --exclude-dir=.git --exclude-dir=test --exclude-dir=__pycache__ 2>/dev/null; then
...
for family in api workspace egress governance ingest writeback migration; do
...
mkdir -p "$WORK_DIR/url-fixture"
printf 'url = "https://api.github.com/repos/owner/repo"\n' > "$WORK_DIR/url-fixture/client.py"
if ! scan "$WORK_DIR/url-fixture" >/dev/null 2>&1; then
  echo '::error::contract-scan: GitHub API URL was treated as a wire code' >&2
  exit 1
fi
printf 'reason = "api.github"\n' >> "$WORK_DIR/url-fixture/client.py"
if scan "$WORK_DIR/url-fixture" >/dev/null 2>&1; then
  echo '::error::contract-scan: bare api.github escaped registration' >&2
  exit 1
fi
